...
// use rust_hdl::dff::DFF;
// use rust_hdl::dff_setup;
// use rust_hdl::ramrom::ram::RAM;
// use rust_hdl::ramrom::sync_rom::SyncROM;
// use rust_hdl_core::prelude::*;
// use rust_hdl_core::signed::ToSignedBits;
...
#[derive(LogicBlock)]
pub struct RolledFir<const ADDR_BITS: usize> {
    ...
    lr_add: DFF<Signed<16>>,
    // The output of the multipy
    ...
    for RolledFir<ADDR_BITS>
{
    ...
    fn update(&mut self) {
        ...
        dff_setup!(self, clock, head_ptr, index, accum, state, lr_add);
        ...
        self.lr_add.d.next = self.left_sample.val() + self.right_sample.val();
        self.multiply_output.next = self.lr_add.q.val() * (self.coeff_memory.data.val());
        self.mac_output.next = signed_bit_cast::<48, 32>(self.multiply_output.val()) + self.accum.q.val();
        if self.state.q.val() == MACFIRState::Idle {
            self.lr_add.d.next = 0.into();
        ...
        match self.state.q.val() {
            ...
            MACFIRState::CenterTap => {
                self.index.d.next = self.index.q.val() + 1;
                self.accum.d.next = self.mac_output.val();
                self.state.d.next = MACFIRState::Finalize;
            }
            MACFIRState::Finalize => {
                self.index.d.next = self.index.q.val() + 1;
                self.accum.d.next = self.mac_output.val();
...
impl<const ADDR_BITS: usize> RolledFir<ADDR_BITS> {
